    ...
    void ReportHandler::report_entry_handler(void *arg)
    {
        ...
        while (1)
        {
            ...
            while (!_report_msg_buffer.empty())
            {
                ...
                /*
                    ======================================
                    Implementacao GAMBIARRA
                    ======================================
                    - Aqui eu vou vou add o valor pwm na estrutura
                    a parte. Porque essa parte não funciona
                */
                local_storage *m_storage = local_storage::GetObjs();
                m_storage->RecordPWMonStorage(pwm_value);
                m_storage = m_storage->GiveObjs();

                MY_LOGI("Valor armazenado no relatorio = %d", (int)pwm_value);
