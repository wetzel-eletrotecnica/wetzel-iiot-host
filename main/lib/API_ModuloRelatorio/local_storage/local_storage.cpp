...
#define MEMORY_LUMINARI_STORAGE "rela_lumi"
    ...
    SemaphoreHandle_t local_storage::_ptr_local_storage_binary = nullptr;
    /**
     * ====================================================
     *                   Public Function
     * ====================================================
    ...
    {
        // Verificar se o semaphoro já existe
        if (_ptr_local_storage_binary == nullptr)
        {
            _ptr_local_storage_binary = xSemaphoreCreateBinary();
            assert(_ptr_local_storage_binary != nullptr);
            xSemaphoreGive(_ptr_local_storage_binary);
        }
        ...
        // Trava a ponteiro pq vai retornar o ponteiro da classe
        xSemaphoreTake(_ptr_local_storage_binary, portMAX_DELAY);
        return _ptr_local_storage;
    }

    local_storage *local_storage::GiveObjs()
    {
        if (_ptr_local_storage_binary)
        {
            xSemaphoreGive(_ptr_local_storage_binary);
        }
        else
        {
            MY_LOGW("GiveObjs on local_storage called with _ptr_local_storage_binary = nullptr");
        }
        return nullptr;
    }
    ...
    std::string local_storage::ReturnDayPointByDayAndMonthIndex(month_type_t month, uint8_t day, uint16_t watts_luminaria, uint8_t qnt_luminarias)
    {
        ...
        if (day < 32)
        {
            ...
            // FIXME: Aqui faltou tratar o que acontece quando é 255 o retorno do vetor
            if (mes_index != 255)
            {
    ...
    void local_storage::UpdateStructWhich10min(void *args)
    {
        ...
        _ptr_local = _ptr_local->GiveObjs();
    }
